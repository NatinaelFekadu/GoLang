...
func main(){
	...
	select{
	case a = <- x:
		fmt.Printf("X finished first, value %v",a)
	case b = <- y:
		fmt.Printf("Y finished first, value %v",b)
	}
}
...
// func main() {
// 	greet()
// 	storeData("This is some dunny data!", "dummy-data.txt")
...
// func storeMoreData(lines int,fileName string,channel chan int){
// 	for i := 0; i < lines; i++ {
